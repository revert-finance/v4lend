...
/// @dev Modes can be combined using bitwise OR (e.g., MODE_AUTO_COMPOUND | MODE_AUTO_RANGE)
library PositionModeFlags {
    uint8 internal constant MODE_NONE = 0;                    // 0b00000000
    uint8 internal constant MODE_AUTO_COMPOUND = 1 << 0;      // 0b00000001 (1)
    ...
    /// @notice Check if AUTO_COMPOUND flag is set
    function hasAutoCompound(uint8 mode) internal pure returns (bool) {
        return hasFlag(mode, MODE_AUTO_COMPOUND);
    }
    ...
    /// @notice Check if mode has any trigger-based flags (not NONE or AUTO_COMPOUND only)
    function hasTriggers(uint8 mode) internal pure returns (bool) {
        // Any mode that sets triggers (not NONE or AUTO_COMPOUND only)
        return (mode & ~MODE_AUTO_COMPOUND) != 0;
    }
    ...
    /// @notice Check if mode is only AUTO_COMPOUND (no triggers)
    function isAutoCompoundOnly(uint8 mode) internal pure returns (bool) {
        return mode == MODE_AUTO_COMPOUND;
    }
}
