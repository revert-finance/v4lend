...
- Hook automation follows a reminted position through `V4Vault.transform` automatically, and through a direct `V4Utils` range change when the caller opts in by setting `increaseLiquidityHookData` to `abi.encodePacked(REMINT_MIGRATION_TAG, oldTokenId)` (tag: `bytes4(keccak256("RevertHookRemintMigration(uint256)"))`); the hook checks that claim against the caller's ERC721 authority over the old token. Without the opt-in the replacement NFT starts with no automation config and the owner calls `RevertHook.setPositionConfig(...)` on it; the old NFT stays with the owner, so its config and any auto-lend shares remain reachable.
- Hook-managed swaps have no default output floor. They are bounded by the oracle window (`setMaxTicksFromOracle`, 100 ticks at deploy) and by the oracle's own pool-deviation guard, not by an `amountOutMin`; owners of large positions should set `RevertHook.setSwapProtectionConfig(...)`. Trigger-driven actions inherit that window from swap traversal; permissionless `autoCollect` is price-checked against it directly, since it never traverses. The assumptions and worst case are recorded at `AUDIT-ACCEPTED-HOOK-SWAP-NO-SLIPPAGE-FLOOR` in [src/hook/RevertHookSwapActions.sol](src/hook/RevertHookSwapActions.sol).
