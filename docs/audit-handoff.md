...
- V4LE-51: `autoExit` re-validates the pair at execution: a debt-bearing vault position whose vault asset is neither pool currency (nor the native alias) - reachable by configuring a directly held NFT and depositing it into such a vault afterwards, which the hook never sees - is skipped before any liquidity moves, with `AutoExitIncompatibleVaultAsset` + `HookActionFailed`, and its zombie config (trigger nodes already popped) is retired; zero-debt exits still run. The trigger is consumed like every failed action (`AUDIT-ACCEPTED-HOOK-TRIGGER-CONSUME`); keeping it armed would let one misconfigured position re-fail on every swap
- V4LE-71: a hook leverage adjustment that sizes to nothing on an off-target loan (`liquidityToRemove` flooring to zero for a position whose raw liquidity is small next to its value, a valueless position, or a zero borrow) is a failed action (`HookActionFailed`, fired node consumed, no re-centring of the trigger window) instead of a success that re-centred the triggers around an unhandled loan; removing one liquidity unit instead would credit nothing
- V4LE-53 / V4LE-21 (external routes): `LiquidityCalculator.calculateSimple` has a pool-reading overload that plans an external route against its real depth: it starts from a constant-liquidity plan at the route's current price and active liquidity, quotes the planned input by walking the route's initialized ticks (SwapMath step by step, liquidity net applied at each crossing, at most 32 crossings), and refines the in-range split twice at the quoted effective price; the caller's output fee is netted out of the planned output. `RevertHookSwapActions.planExternalRoute` (staticcalled by the action sidecars) hands it the route pool and the hook's per-mode `swapFeeBps`, so a shallow route, a route with a liquidity hole at the current tick, or the hook's own output fee no longer leaves a large share of the swap as owner leftover while the replacement is underfunded. A route with no active liquidity plans no swap. The pure nine-argument `calculateSimple` remains as the constant-liquidity core. The same-pool planner (`calculateSamePool`) still ignores the hook output fee - see the deferred note in the audit report
- V4LE-72 (deferred, design): a remint that leaves this hook's pools (different hook, no hook, or another pair; the vault path's `replacementStaysHere == false`, or a direct V4Utils range change the hook never sees) leaves the retired token's carried protocol fee where it is; the token is drained and its zero-liquidity burn runs no liquidity callback, so the remainder is uncollectible. This is the already-accepted "carried fee larger than the final principal is stranded" leak reached through a remint instead of a burn, and the hook alone cannot settle it (the fee is an amount owed, not funds held). Proposed follow-up: an owner-callable `settlePendingProtocolFee(tokenId)` that pulls the owed currencies and forwards them to `protocolFeeRecipient`, with `migrateVaultPosition` refusing an out-of-scope remint while a fee is pending, or bounding what a fee-only collection may defer
