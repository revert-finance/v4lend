...
/// @title DeployBase
/// @notice Deployment script for full Revert V4Utils ecosystem on Base
...
contract DeployBase is Script {
    ...
    // ==================== Chainlink Oracle Feed Addresses ====================
    ...
    // Interest rate model parameters (Compound V2 style)
    ...
    function run() external {
        ...
        console.log("Deploying Revert V4Utils ecosystem on Base...");
        ...
        RevertHookFunctions hookFunctions =
            new RevertHookFunctions(IPermit2(PERMIT2), oracle, ILiquidityCalculator(liquidityCalculator));
        console.log("RevertHookFunctions deployed at:", address(hookFunctions));

        RevertHookFunctions2 hookFunctions2 =
            new RevertHookFunctions2(IPermit2(PERMIT2), oracle, ILiquidityCalculator(liquidityCalculator));
        ...
        V4Vault vault = new V4Vault(
            "Revert Lend USDC", "rlUSDC", USDC, POSITION_MANAGER, interestRateModel, oracle, IWETH9(WETH)
        );
        ...
        vault.setLimits(
            100000, // minLoanSize: 0.1 USDC (6 decimals)
        ...
        FlashloanLiquidator flashloanLiquidator =
            new FlashloanLiquidator(POSITION_MANAGER, UNIVERSAL_ROUTER, ZEROX_ALLOWANCE_HOLDER);
        console.log("FlashloanLiquidator deployed at:", address(flashloanLiquidator));

        // Deploy V4Utils
        V4Utils v4Utils = new V4Utils(POSITION_MANAGER, UNIVERSAL_ROUTER, ZEROX_ALLOWANCE_HOLDER, IPermit2(PERMIT2));
        console.log("V4Utils deployed at:", address(v4Utils));

        // Deploy LeverageTransformer
        LeverageTransformer leverageTransformer =
            new LeverageTransformer(POSITION_MANAGER, UNIVERSAL_ROUTER, ZEROX_ALLOWANCE_HOLDER, IPermit2(PERMIT2));
        console.log("LeverageTransformer deployed at:", address(leverageTransformer));

        // ==================== Step 7: Configure Vault-Transformer Integration ====================
        ...
        console.log("V4Vault:", address(vault));
