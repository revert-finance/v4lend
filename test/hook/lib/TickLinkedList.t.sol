...
contract TickLinkedListTest is Test {
    ...
    function test_popTokenIds_PartialSameTickKeepsNode() public {
        assertTrue(increasingList.insert(100, 1));
        assertTrue(increasingList.insert(100, 2));
        assertTrue(increasingList.insert(100, 3));
        assertTrue(increasingList.insert(200, 4));

        (uint256[] memory popped, bool tickDrained) = increasingList.popTokenIds(100, 2);

        assertFalse(tickDrained);
        assertEq(popped.length, 2);
        assertEq(popped[0], 1);
        assertEq(popped[1], 2);
        assertEq(increasingList.size, 2);
        assertEq(increasingList.head, 100);
        assertEq(increasingList.next[100], 200);
        assertEq(increasingList.tokenIdStart[100], 2);
        assertEq(increasingList.tokenIds[100].length, 3);
        assertEq(increasingList.tokenIds[100][2], 3);
    }

    function test_popTokenIds_DrainingTickUnlinksNode() public {
        assertTrue(increasingList.insert(100, 1));
        assertTrue(increasingList.insert(100, 2));
        assertTrue(increasingList.insert(200, 3));

        (uint256[] memory popped, bool tickDrained) = increasingList.popTokenIds(100, 10);

        assertTrue(tickDrained);
        assertEq(popped.length, 2);
        assertEq(popped[0], 1);
        assertEq(popped[1], 2);
        assertEq(increasingList.size, 1);
        assertEq(increasingList.head, 200);
        assertEq(increasingList.tokenIdStart[100], 0);
        assertEq(increasingList.tokenIds[100].length, 0);
    }
