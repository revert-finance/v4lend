...
import {IUniswapV3Pool} from "src/vault/liquidation/FlashloanLiquidator.sol";
...
contract V4VaultTest is V4ForkTestBase {
    ...
    function _buildChangeRangeInstructions(uint256 tokenId, address recipient)
        internal
        view
        returns (V4Utils.Instructions memory inst)
    {
   ...
   // fuzz testing borrow amount
    ...
    function test_ApproveTransform_RevertWhenCallerIsNotLoanOwner() public {
        _setupBasicLoan(false);

        vm.prank(makeAddr("notOwner"));
        vm.expectRevert(Constants.Unauthorized.selector);
        vault.approveTransform(nft1TokenId, address(v4Utils), true);
    }

    function test_ApproveTransform_RevocationBlocksApprovedCaller() public {
        _setupBasicLoan(false);

        address approvedCaller = makeAddr("approvedCaller");

        vm.prank(nft1Owner);
        vault.approveTransform(nft1TokenId, approvedCaller, true);
        assertTrue(vault.transformApprovals(nft1Owner, nft1TokenId, approvedCaller), "approval should be set");

        vm.prank(nft1Owner);
        vault.approveTransform(nft1TokenId, approvedCaller, false);
        assertFalse(vault.transformApprovals(nft1Owner, nft1TokenId, approvedCaller), "approval should be cleared");

        vm.prank(approvedCaller);
        vm.expectRevert(Constants.Unauthorized.selector);
        vault.transform(nft1TokenId, address(v4Utils), bytes(""));
    }

    function test_Transform_RemintCarriesApprovalOnlyForApprovedCaller() public {
        _setupBasicLoan(true);
        ...
        assertTrue(
            vault.transformApprovals(nft1Owner, newTokenId, approvedCaller),
            "approved caller should keep approval on reminted token"
        );
        assertFalse(
            vault.transformApprovals(nft1Owner, newTokenId, otherCaller),
            "unapproved caller should not gain approval on reminted token"
        );
    }
    ...
    function test_DecreaseLiquidityAndCollect_AllowsHealthyWithdrawal() public {
        ...
        uint128 liquidityBefore = positionManager.getPositionLiquidity(nft1TokenId);
        uint256 usdcBefore = usdc.balanceOf(nft1Owner);
        uint256 wethBefore = weth.balanceOf(nft1Owner);
        ...
        vm.prank(nft1Owner);
        (uint256 amount0, uint256 amount1) = vault.decreaseLiquidityAndCollect(params);

        assertLt(positionManager.getPositionLiquidity(nft1TokenId), liquidityBefore, "liquidity should decrease");
        assertEq(usdc.balanceOf(nft1Owner) - usdcBefore, amount0, "recipient should receive token0 delta");
        assertEq(weth.balanceOf(nft1Owner) - wethBefore, amount1, "recipient should receive token1 delta");
        assertGt(amount0 + amount1, 0, "withdrawal should collect assets");
    }

    function test_DecreaseLiquidityAndCollect_RevertWhenCallerIsNotLoanOwner() public {
        _setupBasicLoan(false);
    ...
    function test_DecreaseLiquidityAndCollect_RevertWhenWithdrawalWouldBreakHealth() public {
        _setupBasicLoan(true);

        uint128 liquidityToRemove = positionManager.getPositionLiquidity(nft1TokenId) / 10;
    ...
    function test_DecreaseLiquidityAndCollect_RevertDuringTransformMode() public {
        _setupBasicLoan(false);

        DecreaseLiquidityDuringTransformTransformer transformer = new DecreaseLiquidityDuringTransformTransformer(vault);
